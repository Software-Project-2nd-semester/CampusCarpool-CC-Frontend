...
const MyReceive = () => {
    ...
    const fetchNotices = async () => {
      try {
        const response = await axios.get(`${process.env.REACT_APP_API_URL}/api/reviews/received`, {
          params: {
            receiverSub: sessionStorage.getItem('sub'),
          },
        });
  ...
  return (
    <div className="container mx-auto p-4">
      <h1 className="text-3xl font-bold text-center mb-6">{nickname}님이 받은 후기</h1>
      <div className="space-y-6">
        {data.map((review, index) => (
          <div key={index} className="bg-white p-6 rounded-lg shadow-lg hover:shadow-xl transition-shadow duration-300">
            ...
            <div onClick={()=>{navigate(`/user/profile/${review.writerSub}`,{state:{sub:review.writerSub}})}}>
              <button className='text-white bg-blue-500 px-2 py-2 rounded-xl'>리뷰 작성자 프로필 보러가기</button>
            </div>
          </div>
        ))}
      </div>
